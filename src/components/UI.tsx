    ...
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-4">
      ...
      <AnimatePresence mode="popLayout">
        {countdown !== null && (
          ...
          >
            <motion.span 
              className="text-[18vw] font-black text-white italic tracking-tighter"
            >
            ...
            <div className="bg-zinc-900/90 p-8 rounded-3xl border border-white/10 shadow-2xl max-w-md w-full flex flex-col items-center gap-6">
              {isDead && (
                <div className="text-center">
                  ...
                  <p className="text-white/60">Final Length: {player ? Math.floor(player.score) : 0}</p>
                </div>
              )}
              ...
              <button
                onClick={startJoinCountdown}
                className="w-full py-4 bg-zinc-100 text-black font-bold rounded-xl hover:bg-white transition-colors active:scale-95 border-b-4 border-zinc-400"
              >
                {isDead ? 'RESPAWN' : 'PLAY'}
              </button>
            </div>
